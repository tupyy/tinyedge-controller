...
func (d *DeviceRepo) GetSet(ctx context.Context, id string) (entity.Set, error) {
	...
	tx := d.getDb(ctx).Table("device_set").
		Select(`device_set.*, device.id as device_id,configuration.*, sets_workloads.manifest_work_id as manifest_id`).
...
func (d *DeviceRepo) GetNamespace(ctx context.Context, id string) (entity.Namespace, error) {
	...
	tx := d.getDb(ctx).Table("namespace").
		Select(`device_set.*,configuration.*, device.id as device_id, device_set.id as device_set_id, namespaces_workloads.manifest_work_id as manifest_id`).
